...
fn report_error_terse_formatted_or_verbose_formatted() {
    let cases = build_cases(
        ...
        &func_2_step(stringify!(
            report_error_terse_formatted_or_verbose_formatted
        )),
    );
...
fn build_cases(expected: [&str; 3], step: &str) -> [(Verbosity, String, String); 3] {
    [
        (
            ...
            format!("{} when {}", step, Verbosity::Quite),
        ),
        (
            Verbosity::Terse,
            expected[1].to_string(),
            format!("{} when {}", step, Verbosity::Terse),
        ),
        (
            Verbosity::Verbose,
            expected[2].to_string(),
            format!("{} when {}", step, Verbosity::Verbose),
        ),
    ]
}
